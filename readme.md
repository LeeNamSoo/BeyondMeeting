...
![image-20220114190015723](readme.assets/image-20220114190015723.png)

- 빽 친구들과 함께 험난한 erd여행을 했다.
  - 가장 복잡했던 부분인 모자의 처리를 해결해서 한시름 놨지만 아직 빠뜨린 부분이 있을 수 도...
  - 복잡할 수 있는 부분은 회의(meeting)와 모자(hat)의 관계에서 기법별 모자를 선택하거나 하지 않을 수도 있기 때문에 mandatory를 해제해준 부분이 있다.
  - 처음 모자테이블을 sixhat으로 column 을 색깔별 6개로 나누었을때  상당히 복잡해질뻔 했지만 테이블을 수정해서 잘 바꿀수 있었다.
  - 관계를 보면
    - 회의와 회의록 관계만 유일하게 1대1 관계
    - 팀속에 회의가 여럿이기 때문에 1대다의 관계를 가졌고
    - 유저와 팀은 다대 대 관계로 팀에 다양한 유저들이 모이고 유저는 또 다양한 팀을 가질 수 있기 때문에 다대다의 관계로 작성
    - 유저와 회의의 관계도 마찬가지
    - 유저와 모자의 관계는 조금 복잡한데
      - 유저와 모자는 다대 다의 관계이다. 표시하고자 하는 모자의 갯수 자체가 6개이므로(마이페이지경우)모자는 당연히 다고 유저또한 유저 마다마다의 모자가 다르다.
    - 회의와 모자는 회의 1에 모자 n으로 1대다의 관계이다.(다만 회의 기법에 의해 모자가 0이 될 수 있다.)

figma를 이용한 후론트 

![image-20220114191619507](readme.assets/image-20220114191619507.png)

각각 메인페이지 메인페이지 관리페이지 마이페이지 이다.

현재는 회의하는 페이지는 구성하지 못하고 종료된 상태고 

가장 문제되는 부분은 팀내 회의록에 대한 의견합일과 백에서 페이지에 가져올 데이터를 정리할 필요가 있다고 느꼈다.

기법에 따른 달라지는 관리페이지의 모습과 마이페이지의 모습또한 고려대상이 된다.
